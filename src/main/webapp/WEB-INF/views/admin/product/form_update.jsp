...
<main>
    <form action="/admin/product/update?id=${product.id}" method="post" class="container">
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <select name="category">
                <c:forEach items="${listCategory}" var="ct">
                    <option value="${ct.id}" ${ct.id == product.category.id ? 'selected="selected"' : ''}>${ct.name}</option>
                </c:forEach>
            </select>
        </div>
